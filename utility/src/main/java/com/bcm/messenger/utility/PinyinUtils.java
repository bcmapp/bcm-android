...
public class PinyinUtils {
    ...
    protected static String[] pystr = new String[]{"a", "ai", "an", "ang", "ao", "ba", "bai", "ban", "bang", "bao", "bei", "ben", "beng", "bi", "bian",
    ...
    public String getSpellingForUpper(String chs) {
        try {
            return spell(chs).toUpperCase(Locale.ENGLISH);
        } catch (Exception ex) {
            ALog.e(TAG, "", ex);
        }
        return chs.toUpperCase(Locale.ENGLISH);
    }

    public String getSpelling(String chs) {
        try {
            return spell(chs).toLowerCase(Locale.ENGLISH);
        } catch (Exception ex) {
            ALog.e(TAG, "", ex);
        }
        return chs.toLowerCase(Locale.ENGLISH);
    }

    private String spell(String content) throws Exception {
        if (!checkNeedSpell(content)) {
            return content;
        }
        String key, value;
        buffer = new StringBuilder();
        for (int i = 0; i < content.length(); i++) {
            key = content.substring(i, i + 1);
            if (key.getBytes(StandardCharsets.UTF_8).length >= 2) {
                value = convert(key);
            } else {
                value = key;
            }
    ...
    private boolean checkNeedSpell(String content) {
        //判断是否是汉字
