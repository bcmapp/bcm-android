...
object AmeURLUtil {
    ...
    private val httpUrlRegex = Pattern.compile("^(https?)://[-a-zA-Z0-9+&@#/%?=~_|!:,.;]*[-a-zA-Z0-9+&@#/%=~_|]")
    private val httpWWWUrlRegex = Pattern.compile("^(www.)[-a-zA-Z0-9+&@#/%?=~_|!:,.;]*[-a-zA-Z0-9+&@#/%=~_|]")
     ...
     * @hide
     */
    internal val IANA_TOP_LEVEL_DOMAINS = (
            "(?:"
     ...
     * [.IP_ADDRESS]
     */
    private val STRICT_DOMAIN_NAME = ("(?:" + STRICT_HOST_NAME + "|"
            + IP_ADDRESS_STRING + ")")
    ...
    private val AUTOLINK_WEB_URL_PATTERN = Pattern.compile("($WEB_URL_WITH_PROTOCOL|$WEB_URL_WITHOUT_PROTOCOL)")
     ...
     * @return Boolean
     */
    fun isLegitimateUrl(url: String?): Boolean {
        return if(url == null) {
            false
        }else {
            try {
                val trimUrl = url.trim()
                val matcher = AUTOLINK_WEB_URL_PATTERN.matcher(trimUrl)

                while(matcher.find()) {
                    if(matcher.group() == trimUrl){
                        return true
                    }
                }

            }catch (ex: Exception) {
                ALog.e("AmeURLUtil", "isLegitimateUrl error", ex)
            }
            false
        }
    }
     ...
     * get web logo
     */
    fun getUrlLogo(url: String): String {
        try {
            var n = url.trim()
            ...
            return "${urlObj.protocol}://${urlObj.host}$port/favicon.ico"
