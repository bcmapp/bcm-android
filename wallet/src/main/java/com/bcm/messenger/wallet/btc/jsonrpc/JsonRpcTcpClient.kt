...
data class TcpEndpoint(val host: String, val port: Int, val cert: String?)
...
//    private val ssf = SSLSocketFactory.getDefault() as SSLSocketFactory
    ...
    @Volatile
    private var currentResponseTimeout = SMALL_RESPONSE_TIMEOUT
    @Volatile
    ...
    fun start() {
        ...
        thread(start = true) {
            while (!isStopped) {
                ...
                try {
                    synchronized(this) {
                        socket = if (currentEndpoint.cert == null || currentEndpoint.cert.isEmpty()) {
                            SSLSocketFactory.getDefault().createSocket()
                        } else {
                            SslUtils.getSsLSocketFactory(currentEndpoint.cert).createSocket()
                        }
//                        socket = ssf.createSocket()
                        socket?.apply {
                            soTimeout = currentResponseTimeout.toInt()
                ...
                currentResponseTimeout = calculateNewTimeout(connectionAttempt)
                pingTimer?.cancel()
            }
        }
    }
    ...
    fun write(requests: List<RpcRequestOut>): BatchedRpcResponse {
        ...
        requests.forEach {
            ...
            //logger.logInfo("requests id: ${it.id}, method name: ${it.methodName}, version: ${it.version}")
        }

        val compoundId = compoundId(requests.map { it.id.toString() }.toTypedArray())

        logger.logInfo("request compoundId: $compoundId")
        callbacks[compoundId] = {
            logger.logInfo("receive callback: ${it.toString()}, compoundId: $compoundId")
        ...
        currentResponseTimeout = calculateNewTimeout(executedIn)
...
//        logger.logInfo("messageReceived: $message")
...
//        logger.logInfo("internalWrite msg: $msg")
        thread(start = true) {
            try {
                val bytes = (msg + "\n").toByteArray()
