...
public class AddressEncoder {
    ...
    public static AddressEncoder decodeERC(String s) throws IOException {
        ...
        for (String entry : parsed) {
            ...
            if (entry_s.length != 2)
                continue;
            if (entry_s[0].equalsIgnoreCase("value"))
                re.amount = entry_s[1];
            if (entry_s[0].equalsIgnoreCase("gas"))
                re.gas = entry_s[1];
            if (entry_s[0].equalsIgnoreCase("data"))
                re.data = entry_s[1];
        }
        return re;
    }
    ...
    public static AddressEncoder decodeICAP(String s) throws IOException {
        ...
        for (String entry : parsed) {
            ...
            if (entry_s.length != 2)
                continue;
            if (entry_s[0].equalsIgnoreCase("amount"))
                re.amount = entry_s[1];
            if (entry_s[0].equalsIgnoreCase("gas"))
                re.gas = entry_s[1];
            if (entry_s[0].equalsIgnoreCase("data"))
    ...
    public static AddressEncoder decodeLegacyLunary(String s) throws IOException {
        if (!s.startsWith("iban:") && !s.startsWith("IBAN:"))
            return new AddressEncoder(s);
