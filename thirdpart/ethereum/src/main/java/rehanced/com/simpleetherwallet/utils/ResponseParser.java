...
public class ResponseParser {
    ...
    public static ArrayList<TransactionDisplay> parseTransactions(String response, String walletname, String address, byte type) {
        try {
            ...
            for (int i = 0; i < data.length(); i++) {
                ...
                String input = data.getJSONObject(i).optString("input");
                //                if(input != null) {
                //                    try {
                //                        input = new String(StringUtil.INSTANCE.hexToBytes(Numeric.cleanHexPrefix(input)));
                //                    }catch (Exception ex) {
                //                        ex.printStackTrace();
                //                    }
                //                }
    ...
    public static ArrayList<WalletDisplay> parseWallets(String response, List<StorableWallet> storedwallets, Context context) throws Exception {
