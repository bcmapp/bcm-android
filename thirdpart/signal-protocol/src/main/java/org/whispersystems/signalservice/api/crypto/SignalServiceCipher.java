...
/**
 * This is used to decrypt received {@link org.whispersystems.signalservice.internal.push.SignalServiceProtos.Envelope}s.
 *
...
public class SignalServiceCipher {
  ...
  /**
   * Decrypt a received {@link org.whispersystems.signalservice.internal.push.SignalServiceProtos.Envelope}
   *
   * @param envelope The received SignalServiceProtos.Envelope
   *
   ...
   */
  public SignalServiceContent decrypt(SignalServiceProtos.Envelope envelope)
             ...
             LegacyMessageException, NoSessionException
  {
    try {
      ...
      if (envelope.hasLegacyMessage()) {
        DataMessage message = DataMessage.parseFrom(decrypt(envelope, envelope.getLegacyMessage().toByteArray()));
        content = new SignalServiceContent(createSignalServiceMessage(envelope, message));
      } else if (envelope.hasContent()) {
        Content message = Content.parseFrom(decrypt(envelope, envelope.getContent().toByteArray()));
  ...
  private SignalServiceGroup createGroupInfo(SignalServiceProtos.Envelope envelope, DataMessage content) {
