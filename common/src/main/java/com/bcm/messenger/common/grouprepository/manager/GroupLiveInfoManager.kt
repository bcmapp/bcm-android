...
object GroupLiveInfoManager:AccountContextMap<GroupLiveInfoManager.GroupLiveInfoManagerImpl>({
    GroupLiveInfoManagerImpl(it)
}) {
    class GroupLiveInfoManagerImpl internal constructor(private val accountContext: AccountContext) {
            ...
            if (groupLiveInfo.isLiveStatus) {
                ...
                if (finishTimeDelay <= 0) {
                    ...
                    AmeDispatcher.singleScheduler.scheduleDirect( {
                        executeAutoStopLiveTask(getCurrentLiveInfo(groupLiveInfo.gid))
                    }, finishTimeDelay, TimeUnit.MILLISECONDS)
                }
            }
        }
