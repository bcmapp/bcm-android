...
public class SmsDatabase extends MessagingDatabase {
    ...
    public long getLastCannotDecryptMessage(long threadId) {
        SQLiteDatabase db = databaseHelper.getReadableDatabase();
        String selection = "(" + TYPE + " & " + Types.BASE_TYPE_MASK + ") = " + Types.BASE_FAIL_CANNOT_DECRYPT_TYPE + " AND " + THREAD_ID + " = " + String.valueOf(threadId);
            ...
            while (cursor.moveToNext()) {
                if (Types.isOutgoingMessageType(cursor.getLong(cursor.getColumnIndexOrThrow(TYPE)))) {
                    ...
                    if (ourAddress.equals(theirAddress)) {
                        ...
                        updateTypeBitmask(id, Types.BASE_TYPE_MASK, Types.BASE_FAIL_CANNOT_DECRYPT_TYPE);
                        return true;
                    }
                }
            }
    ...
    public Pair<Long, Long> copyMessageInbox(long messageId) {
        ...
        trimThreadJob(record.getThreadId());
    ...
    public void markCallDetail(long id, long duration, int communicationType) {
        ...
        contentValues.put(COMMUNICATION_TYPE, communicationType);
        SQLiteDatabase db = databaseHelper.getWritableDatabase();
        db.update(TABLE_NAME, contentValues, ID_WHERE, new String[]{String.valueOf(id)});
    ...
    public @NonNull
    Pair<Long, Long> insertReceivedCall(@NonNull Address address, long duration, int communicationType) {
        return insertCallLog(address, Types.INCOMING_CALL_TYPE, false, duration, communicationType);
    }

    public @NonNull
    Pair<Long, Long> insertOutgoingCall(@NonNull Address address, long duration, int communicationType) {
        return insertCallLog(address, Types.OUTGOING_CALL_TYPE, false, duration, communicationType);
    }

    public @NonNull
    Pair<Long, Long> insertIncomingMissedCall(@NonNull Address address) {
        return insertCallLog(address, Types.MISSED_INCOMING_CALL_TYPE, true, 0, 0);
    }

    public @NonNull
    Pair<Long, Long> insertOutgoingMissedCall(@NonNull Address address) {
        return insertCallLog(address, Types.MISSED_OUTGOING_CALL_TYPE, true, 0, 0);
    }
    ...
    Pair<Long, Long> insertCallLog(@NonNull Address address, long type, boolean unread, long duration, int communicationType) {
        ...
        values.put(COMMUNICATION_TYPE, communicationType);
    ...
    protected Optional<InsertResult> insertMessageInbox(IncomingTextMessage message, long type) {
        ...
        } else if (message.isPreKeyBundle() && !message.isLocation()) {
        ...
        try {
            ...
            values.put(DATE_RECEIVED, message.getReceivedTimestamp());
            ...
            values.put(PAYLOAD_TYPE, message.getPayloadType());
            values.put(TYPE, type);
            values.put(THREAD_ID, threadId);
            ...
            if (message.isPush() && isDuplicate(message, threadId)) {
                ...
                if (message.getSubscriptionId() != -1) {
                    ...
                    if (rdb != null) {
                        rdb.setDefaultSubscriptionId(recipient, message.getSubscriptionId());
                    }
                }
    ...
    protected long insertMessageOutbox(long threadId, OutgoingTextMessage message,
        ...
        contentValues.put(PAYLOAD_TYPE, message.getPayloadType());
        contentValues.put(DATE_RECEIVED, System.currentTimeMillis());
    ...
    public Cursor getLinkMessagesByConversation(long threadId) {
        String where = PAYLOAD_TYPE + " = 7" + " AND " + THREAD_ID + " = " + threadId;
        SQLiteDatabase db = databaseHelper.getReadableDatabase();
        return db.query(TABLE_NAME, MESSAGE_PROJECTION, where, null, null, null, MmsSmsColumns.NORMALIZED_DATE_SENT + " DESC");
    }
    ...
    public Cursor getBackwardMessages(long threadId, long messageId, int size) {
        SQLiteDatabase db = databaseHelper.getReadableDatabase();
        String limit = 0 + "," + size;
        String where = "(thread_id = ? AND _id > ?)";
        Cursor cursor = db.query(TABLE_NAME, MESSAGE_PROJECTION, where, new String[]{String.valueOf(threadId), String.valueOf(messageId)},
                null, null, null, limit);
        return cursor;
    }

    public Cursor getForwardMessages(long threadId, long messageId, int size) {
        SQLiteDatabase db = databaseHelper.getReadableDatabase();
        String limit = 0 + "," + size;
        String where = "thread_id = ? and _id < ?";
        return db.query(TABLE_NAME, MESSAGE_PROJECTION, where, new String[]{String.valueOf(threadId), String.valueOf(messageId)},
    ...
    public boolean deleteMessagesExcept(long threadId, long... messageIds) {
        ...
        SQLiteDatabase db = databaseHelper.getWritableDatabase();
        db.execSQL("DELETE FROM " + TABLE_NAME + " WHERE _id not in (" + builder.toString() + ") and " + THREAD_ID + " = " + threadId);
        boolean threadDeleted = DatabaseFactory.getThreadDatabase(context).update(threadId, false);
    ...
    public boolean deleteIncomingMessageByDateSent(long threadId, long dateSent) {
        ...
        cursor = db.query(TABLE_NAME, new String[]{ID, TYPE}, where, new String[]{threadId + ""}, null, null, null);
