...
public class PersistentBlobProvider {
    ...
    private static final String URI_STRING = "content://" + BuildConfig.BCM_APPLICATION_ID + "/capture-new";
    public static final Uri CONTENT_URI = Uri.parse(URI_STRING);
    public static final String AUTHORITY = BuildConfig.BCM_APPLICATION_ID;
    ...
    public Uri create(MasterSecret masterSecret,
    ...
    private Uri create(MasterSecret masterSecret,
                       @NonNull InputStream input,
                       long id,
                       @NonNull String mimeType,
                       @Nullable String fileName,
        ...
        String path = masterSecret != null ? getEncryptedFileName(masterSecret, fileName) : fileName;
    ...
    private void persistToDisk(final MasterSecret masterSecret, final long id, final InputStream input) {
        Util.newMasterSecretExecutor().submit(new Runnable() {
            @Override
            public void run() {
                try {
    ...
    public @NonNull
    InputStream getStream(MasterSecret masterSecret, long id) throws IOException {
    ...
    private @Nullable
    String getEncryptedFileName(@NonNull MasterSecret masterSecret, @Nullable String fileName) {
        if (fileName == null) return null;
        return new MasterCipher(masterSecret).encryptBody(fileName);
    }

    public static @Nullable
    String getMimeType(@NonNull Context context, @NonNull Uri persistentBlobUri) {
    ...
    public static @Nullable
    String getFileName(@NonNull Context context, @NonNull MasterSecret masterSecret, @NonNull Uri persistentBlobUri) {
        if (!isAuthority(context, persistentBlobUri)) return null;
