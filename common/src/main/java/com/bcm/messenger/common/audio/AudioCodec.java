...
public class AudioCodec {
    ...
    private AtomicLong playTime = new AtomicLong();
    private IRecordFinished finishListener;
    ...
    public Long getPlayTime() {
        if (playTime.get() == 0 && playStartTime > 0) {
            return System.currentTimeMillis() - playStartTime;
        }
        return playTime.get();
    }

    public void start(final OutputStream outputStream, long maxPlayTime) {
        new Thread(() -> {
            if (playStartTime == 0) {
            ...
            try {
                while (true) {
                    ...
                    boolean timeReached = (System.currentTimeMillis() - playStartTime >= maxPlayTime);
                    if (!running || timeReached) break;
                }

                playTime.set(System.currentTimeMillis() - playStartTime);
            } catch (Throwable e) {
                ALog.e(TAG, "start ", e);
            } finally {
                try {
    ...
    private synchronized void setFinished() {
        ...
        IRecordFinished listener = this.finishListener;
        if (null != listener && !cancel) {
            listener.onRecordFinished(playTime.get());
        }
    }
    ...
    public interface IRecordFinished {
        void onRecordFinished(long playTime);
    }
}
