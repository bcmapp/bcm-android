...
object GroupLogic {
    ...
    private val groupMemberSync = GroupMemberSyncManager()

    private val mGroupFinder = BcmGroupFinder()

    private val memberLoader = GroupMemberInfoNetLoader()
    private val groupAvatarLogic = GroupAutoGenerateLogic()
    ...
    fun init() {
        GroupMessageLogic.init()
        groupCache.init()
        GroupKeyRotate.initRotate()

        BcmFinderManager.get().registerFinder(mGroupFinder)
        BcmFinderManager.get().registerFinder(BcmGroupMemberFinder())
        BcmFinderManager.get().registerFinder(BcmThreadFinder())
    }

    fun unInit() {
        GroupMessageLogic.unInit()
    ...
    private fun createGroupV3(name: String,
        ...
        GroupManagerCore.queryRecipientsInfo(inviteList)
                .subscribeOn(AmeDispatcher.ioScheduler)
                .observeOn(AmeDispatcher.ioScheduler)
                .map { list ->
                    ...
                    stash.validList = list.filter {
                        val recipient = Recipient.from(AppContextHolder.APP_CONTEXT, Address.fromSerialized(it.uid), true)
                ...
                .flatMap { stash ->
                    val ownerSecretString = BCMEncryptUtils.generateMyEncryptKeyString(stash.groupInfoSecretPlainBytes)
                    ...
                    val shareSettingSignArray = BCMEncryptUtils.signWithMe(AppContextHolder.APP_CONTEXT, EncryptUtils.base64Decode(stash.shareSetting.toByteArray()))
                    ...
                    val shareConfirmSignByteArray = BCMEncryptUtils.signWithMe(AppContextHolder.APP_CONTEXT, format.toByteArray())
                    ...
                    GroupManagerCore.createGroupV3(req)
                            .subscribeOn(AmeDispatcher.ioScheduler)
                            .observeOn(AmeDispatcher.ioScheduler)
                            .map {
                ...
                .flatMap { stash ->
                    ...
                    GroupInfoDataManager.saveGroupKeyParam(dbGroupInfo.gid, dbGroupInfo.currentKeyVersion, dbGroupInfo.currentKey)

                    getThreadDB().getThreadIdFor(Recipient.recipientFromNewGroupId(AppContextHolder.APP_CONTEXT, stash.groupInfo.gid))
                ...
                .map<GroupInfoResult> {
                    GroupMessageLogic.syncOfflineMessage(it.info.gid, it.ackState.lastMid, it.ackState.lastAckMid)
                    it
    ...
    private fun createGroupV2(name: String,
        ...
        GroupManagerCore.queryRecipientsInfo(inviteList)
                ...
                .map<List<IdentityKeyInfo>> { list ->
                    ...
                    val ownerKeySpecString = BCMEncryptUtils.generateMyEncryptKeyString(groupPassword)
                    val ownerSecretString = BCMEncryptUtils.generateMyEncryptKeyString(groupSecret)
                    ...
                    val shareSettingSignArray = BCMEncryptUtils.signWithMe(AppContextHolder.APP_CONTEXT, EncryptUtils.base64Decode(shareSetting.toByteArray()))
                    ...
                    GroupManagerCore.createGroupV2(name,
                            icon,
                            broadcast,
                            shareContent,
                            realInviteList,
                    ...
                    if (it.isSuccess) {
                        ...
                        getThreadDB().getThreadIdFor(Recipient.recipientFromNewGroupId(AppContextHolder.APP_CONTEXT, it.data.gid))
    ...
    private fun inviteStrangerNotify(gid: Long, strangers: List<String>) {
        ...
        GroupMessageLogic.systemNotice(gid, message)
    }
    ...
    fun queryGroupInfo(groupId: Long): Observable<GroupInfo> {
        return queryGroupInfoImpl(groupId)
                ...
                .map {
                    GroupMessageLogic.syncOfflineMessage(it.info.gid, it.ackState.lastMid, it.ackState.lastAckMid)
                    GroupInfoDataManager.queryOneGroupInfo(groupId)
    ...
    fun queryGroupInfoByGids(gidList: List<Long>, result: (succeed: Boolean) -> Unit) {
        ...
        GroupManagerCore.getGroupInfoByGids(gidList)
                .subscribeOn(AmeDispatcher.ioScheduler)
                .observeOn(AmeDispatcher.ioScheduler)
                .map<List<GroupInfoEntity>> {
                    ...
                    GroupManagerCore.queryRecipientsInfo(groupList.mapNotNull { it.owner })
                            .subscribeOn(AmeDispatcher.ioScheduler)
                            .observeOn(AmeDispatcher.ioScheduler)
                            .map {
    ...
    private fun queryGroupInfoImpl(groupId: Long): Observable<GroupInfoResult> {
        return GroupInfoNetLoader.loadGroup(groupId)
                ...
                .flatMap { pair ->
                    GroupManagerCore.queryRecipientsInfo(listOf(pair.first.owner!!))
                            .subscribeOn(AmeDispatcher.ioScheduler)
                            .map {
                ...
                .map {
                    ...
                    val recipient = Recipient.recipientFromNewGroupId(AppContextHolder.APP_CONTEXT, dbGroupInfo.gid)
    ...
    private fun parseGroupInfo(info: GroupInfoEntity, ownerIdentityKey: String?, parseCount: Boolean): GroupInfo {
        info.channel = ""
        val dbGroupInfo = GroupInfoDataManager.queryOneGroupInfo(info.gid) ?: GroupInfo()
        ...
        if (dbGroupInfo.isNewGroup) {
            val lastKeyVersion = GroupInfoDataManager.queryLastGroupKeyVersion(dbGroupInfo.gid)
    ...
    fun inviteMember(groupId: Long, memberList: List<String>, result: (succeed: Boolean, succeedList: ArrayList<AmeGroupMemberInfo>?, resultMessage: String?) -> Unit) {
        ...
        GroupManagerCore.queryRecipientsInfo(memberList)
                .subscribeOn(AmeDispatcher.ioScheduler)
                .observeOn(AmeDispatcher.ioScheduler)
                .map { list ->

                    val dbGroupInfo = GroupInfoDataManager.queryOneGroupInfo(groupId)
                    ...
                    if (stash.friendList.isNotEmpty()) {
                        ...
                        GroupMemberCore.inviteMemberJoinGroup(groupId, members, listKeys, proofList, listSecret, signatures)
                                .subscribeOn(AmeDispatcher.ioScheduler)
                                .observeOn(AmeDispatcher.ioScheduler)
                                .map {
                ...
                .map { stash ->
                    ...
                    if (stash.friendList.isNotEmpty()) {
                        ...
                        if (dbGroupInfo.role == AmeGroupMemberInfo.OWNER || dbGroupInfo.needOwnerConfirm == 0) {
                            UserDataManager.insertGroupMembers(memberInfos)
                        }
                    }
    ...
    fun updateGroupAvatar(gid: Long, avatar: String, result: (isSuccess: Boolean, error: String?) -> Unit): Boolean {
        ...
        GroupManagerCore.updateGroup(gid, null, avatar)
                .subscribeOn(AmeDispatcher.ioScheduler)
                .observeOn(AmeDispatcher.ioScheduler)
                .map {
    ...
    fun leaveGroup(groupId: Long, newOwnerUid: String?, result: (succeed: Boolean, error: String) -> Unit) {
        ...
        GroupManagerCore.leaveGroup(groupId, groupInfo.newGroup, newOwnerUid)
                .subscribeOn(AmeDispatcher.ioScheduler)
                .observeOn(AmeDispatcher.ioScheduler)
                .map {
    ...
    private fun leaveSucceed(groupId: Long) {
        AmeDispatcher.io.dispatch {
            ...
            Repository.getRecipientRepo()?.leaveGroup(listOf(groupId))
            Repository.getThreadRepo().cleanConversationContentForGroup(getThreadId(Recipient.recipientFromNewGroupId(AppContextHolder.APP_CONTEXT, groupId)), groupId)
        }
    }
    ...
    fun getGroupMemberInfos(groupId: Long, uidList: List<String>): Observable<List<AmeGroupMemberInfo>> {
        return Observable.just(uidList.toMutableList())
                ...
                .observeOn(AmeDispatcher.ioScheduler)
                .flatMap { queryList ->
                    val localList = UserDataManager.queryGroupMemberList(groupId, uidList)
    ...
    fun muteGroup(groupId: Long, mute: Boolean, callback: (success: Boolean, msg: String?) -> Unit) {
        ...
        GroupManagerCore.updateMyNameAndMuteSetting(groupId, mute, null, null, null)
                .subscribeOn(AmeDispatcher.ioScheduler)
                .observeOn(AmeDispatcher.ioScheduler)
                .doOnNext {
                    if (it.isSuccess) {
    ...
    fun updateMyMemberInfo(gid: Long, name: String?, customName: String?, keyConfig: AmeGroupMemberInfo.KeyConfig?, result: (succeed: Boolean, error: String) -> Unit) {
        ...
        GroupManagerCore.updateMyNameAndMuteSetting(gid, null, encryptName, encryptCustomName, encryptProfileKey)
                .subscribeOn(AmeDispatcher.ioScheduler)
                .observeOn(AmeDispatcher.ioScheduler)
                .map {
                    if (it.isSuccess) {
    ...
    fun deleteMember(groupId: Long, list: ArrayList<AmeGroupMemberInfo>, result: (succeed: Boolean, succeedList: ArrayList<AmeGroupMemberInfo>?, error: String?) -> Unit) {
        ...
        GroupMemberCore.kickGroupMember(groupId, groupInfo.newGroup, uidList)
                .subscribeOn(AmeDispatcher.ioScheduler)
                .observeOn(AmeDispatcher.ioScheduler)
                .map {
                    ...
                    for (delete in uidList) {
                        UserDataManager.deleteMember(groupId, delete)
                    }
                }
                .observeOn(AmeDispatcher.mainScheduler)
                .subscribe({
    ...
    private fun getThreadDB(): ThreadRepo {
        return Repository.getThreadRepo()
    }
    ...
    fun onEvent(e: GroupMemberChangedNotify) {
                ...
                queryGroupInfo(change.groupId) { ameGroup, ackState, error ->
                    AmeDispatcher.io.dispatch {
                        if (null != ameGroup && null != ackState) {
                            ...
                            GroupMessageLogic.syncOfflineMessage(ameGroup.gid, ackState.lastMid, ackState.lastAckMid)
                            getThreadDB().getThreadIdFor(Recipient.recipientFromNewGroup(AppContextHolder.APP_CONTEXT, ameGroup))
                        } else {
        ...
        AmeDispatcher.mainThread.dispatch {
            ...
            if (null != model) {
                when (change.action) {
                    AmeGroupMemberChanged.JOIN -> {
                        GroupMessageLogic.syncJoinReqMessage(change.groupId)
    ...
    fun onEvent(event: GroupRefreshKeyEvent) {
        ...
        queryGroupInfo(event.gid) { ameGroup, ackState, error ->
            AmeDispatcher.io.dispatch {
                if (null != ameGroup && null != ackState) {
                    val groupKey = BCMEncryptUtils.decryptGroupPassword(event.groupKey).first
                    val groupSecret = BCMEncryptUtils.decryptGroupPassword(event.groupInfoSecret).first
    ...
    fun onEvent(event: GroupJoinReviewRequestEvent) {
        ...
        GroupMessageLogic.syncJoinReqMessage(event.gid)
    }

    @Subscribe
    fun onEvent(event: GroupShareSettingRefreshEvent) {
        val owner = GroupInfoDataManager.queryOneGroupInfo(event.gid)?.owner
        if (owner == null || owner.isEmpty()) {
            ALog.w(TAG, "GroupShareSettingRefreshEvent ${event.gid}  group info is null")
            return
        }

        GroupManagerCore.queryRecipientsInfo(listOf(owner))
                .subscribeOn(AmeDispatcher.ioScheduler)
                .observeOn(AmeDispatcher.ioScheduler)
                .map {
                    if (it.isNotEmpty()) {
                        val groupInfo = GroupInfoDataManager.queryOneGroupInfo(event.gid)
                                ?: return@map
    ...
    fun autoReviewJoinRequest(gid: Long, list: List<GroupJoinRequestInfo>, result: (succeed: Boolean, error: String) -> Unit) {
        AmeDispatcher.io.dispatch {
            ...
            GroupManagerCore.autoReviewJoinRequest(groupInfo.gid, reviewList)
                    .subscribeOn(AmeDispatcher.ioScheduler)
                    .observeOn(AmeDispatcher.ioScheduler)
                    .subscribe({
                        result(true, "")
    ...
    fun reviewJoinRequest(gid: Long, list: List<BcmReviewGroupJoinRequest>, result: (succeed: Boolean, error: String) -> Unit) {
        AmeDispatcher.io.dispatch {
            ...
            if (null != groupInfo && !TextUtils.isEmpty(groupInfo.infoSecret)) {
                ...
                val reqList = BcmGroupJoinManager.getJoinInfoByUidList(gid, reqUidList)
                for (i in reqList) {
                ...
                GroupManagerCore.reviewJoinRequestByOwner(groupInfo.gid, reviewList, groupInfo.isNewGroup)
                        .subscribeOn(Schedulers.io())
                        .observeOn(Schedulers.io())
                        .map {
                            BcmGroupJoinManager.updateJoinRequests(reqList)
                        }
                        .doOnError {
    ...
    fun fetchJoinRequestList(gid: Long, delay: Long = 0, startUid: String, count: Long, fetchResult: (succeed: Boolean, list: List<GroupJoinRequestInfo>) -> Unit) {
        ...
        GroupManagerCore.queryPendingList(gid, startUid, count)
                .subscribeOn(Schedulers.io())
                .observeOn(Schedulers.io())
                ...
                .flatMap { result ->
                    ...
                    GroupManagerCore.queryRecipientsInfo(uidList.toList())
                            .map { list ->
    ...
    private fun saveJoinRequestList(gid: Long, list: ArrayList<GroupJoinRequestInfo>) {
        ...
        for (v in map.values) {
            ...
            if (!TextUtils.isEmpty(inviter)) {
                ...
                val exist = BcmGroupJoinManager.getJoinInfoByDetail(gid, joinList[0], v[0].status, v[0].inviter)
                if (exist != null) {
                    exist.comment = v[0].comment
                    exist.timestamp = v[0].timestamp
                    BcmGroupJoinManager.updateJoinRequests(listOf(exist))
            ...
            val mid = GroupMessageLogic.systemNotice(gid, message, false)

            for (r in v) {
                r.updateMid(mid)
            }
        }
        BcmGroupJoinManager.saveJoinGroupInfos(list.filter { it.mid != 0L })
    }

    private fun saveJoinRequestName(req: GroupJoinRequestInfo) {
        val recipient = Recipient.from(AppContextHolder.APP_CONTEXT, Address.fromSerialized(req.uid), false)
        if (recipient.bcmName == null) {
            try {
                ...
                if (!TextUtils.isEmpty(comment.name)) {
                    Repository.getRecipientRepo()?.updateProfileName(recipient, comment.name)
                }
            } catch (e: Exception) {
        ...
        groupMemberSync.syncGroupMember(gid, fromUid, fromTime, listOf(AmeGroupMemberInfo.OWNER, AmeGroupMemberInfo.MEMBER, AmeGroupMemberInfo.ADMIN)) { firstPage, finish, list ->
            if (firstPage) {
                ...
                if (owner?.isNotEmpty() == true) {
                    ...
                    UserDataManager.insertGroupMember(ownerMember)
                }
            }

            if (list.isNotEmpty()) {
                UserDataManager.insertGroupDbMembers(list)
            }
    ...
    fun checkAndSyncGroupMemberList(gid: Long, sync: (isFirstPage: Boolean, allFinished: Boolean) -> Unit) {
        AmeDispatcher.io.dispatch {
            ...
            if (groupCache.getGroupInfo(gid)?.memberSyncState == GroupMemberSyncState.SYNING) {
                val groupMember = UserDataManager.getLastMember(gid)
        ...
        if (diffList.isNotEmpty()) {
            for (kicked in diffList) {
                ...
                if (getThreadDB().getThreadIdIfExist(GroupUtil.addressFromGid(kicked).serialize()) > 0) {
                    MessageDataManager.systemNotice(kicked, AmeGroupMessage.SystemContent(AmeGroupMessage.SystemContent.TIP_GROUP_ILLEGAL))
                }
            }
        }
    }
    ...
    fun checkShareCodeStatus(gid: Long,
        ...
        GroupManagerCore.checkQrCodeValid(gid, shareSig)
                .subscribeOn(Schedulers.io())
                .observeOn(AmeDispatcher.mainScheduler)
                .subscribe({
                    if (it) {
    ...
    fun checkJoinGroupNeedConfirm(gid: Long,
        ...
        GroupManagerCore.checkJoinGroupNeedConfirm(gid)
                .subscribeOn(Schedulers.io())
                .observeOn(Schedulers.io())
                .doOnError {
    ...
    fun joinGroupByShareCode(gid: Long, shareCode: String, shareSig: String, ephemeralKey: ByteArray? = null, comment: String? = null, result: (succeed: Boolean, error: String) -> Unit) {
        Observable.create<ByteArray> {
            ...
            it.onComplete()
                ...
                .observeOn(AmeDispatcher.ioScheduler)
                .flatMap {
                    GroupManagerCore.joinGroupByCode(gid, shareCode, shareSig, comment
                            ?: "", it.size == 64)
                            .subscribeOn(AmeDispatcher.ioScheduler)
                            .observeOn(AmeDispatcher.ioScheduler)
                }.flatMap {
        ...
        Observable.create<String> {
            ...
            it.onComplete()
                ...
                .flatMap { encryptJson ->
                    ...
                    GroupManagerCore.createGroupShareShortUrl(encryptJson)
                }
                .observeOn(Schedulers.io())
                .map { index ->
                    ...
                    GroupInfoDataManager.updateGroupShareShortLink(groupId, shareLink)
                    shareLink
                }
                .observeOn(AmeDispatcher.mainScheduler)
                .subscribe({ shortLink ->
            ...
            if (null != groupInfo) {
                if (TextUtils.isEmpty(groupInfo.shareCodeSetting)) {
                    ...
                    updateShareSetting(gid, false) { succeed, shareCode, error ->
                        if (!succeed) {
                            AmeDispatcher.io.dispatch {
                                val tmp = GroupInfoDataManager.queryOneGroupInfo(gid)
                                if (null != tmp) {
                ...
                GroupManagerCore.updateJoinConfirmSetting(gid, confirm, shareConfirmSign)
                        .subscribeOn(Schedulers.io())
                        .observeOn(Schedulers.io())
                        .doOnNext {
    ...
    private fun broadcastShareSettingRefresh(gid: Long) {
        ...
        if (groupInfo?.role == AmeGroupMemberInfo.OWNER) {
            ...
            GroupMessageLogic.messageSender.sendMessage(gid, AmeGroupMessage(AmeGroupMessage.GROUP_SHARE_SETTING_REFRESH, message), false) {
    ...
    fun updateShareSetting(gid: Long, enable: Boolean, result: (succeed: Boolean, shareCode: String, error: String?) -> Unit) {
        ...
        AmeDispatcher.io.dispatch {
            ...
            if (null != groupInfo && !TextUtils.isEmpty(groupInfo.infoSecret)) {
                ...
                GroupManagerCore.updateShareCodeSetting(gid,
    ...
    fun queryTopMemberInfoList(groupId: Long, count: Long): Observable<ArrayList<AmeGroupMemberInfo>> {
        return GroupMemberCore.getGroupMemberByPage(groupId, listOf(AmeGroupMemberInfo.OWNER, AmeGroupMemberInfo.MEMBER), "", 0, count)
    ...
    fun syncGroupKeyList(gid: Long, versions: List<Long>): Observable<Boolean> {
        val syncList = versions.split(10).map {
            Observable.just(it)
                    ...
                    .observeOn(AmeDispatcher.ioScheduler)
                    .flatMap { list ->
                        GroupManagerCore.getGroupKeys(gid, list)
                                .subscribeOn(AmeDispatcher.ioScheduler)
                    }
                    .observeOn(AmeDispatcher.ioScheduler)
                    .flatMap {
    ...
    private fun saveGroupKey(groupInfo: GroupInfo, keyContent: GroupKeysContent.GroupKeyContent) {
        ...
        when (GroupKeyMode.ofValue(keyContent.keyMode)) {
            GroupKeyMode.STRONG_MODE -> {
                val contentKey = keyContent.strongModeKey ?: throw Exception("strong key is null")

                val key = StrongKeyExchangeParam.strongKeyContentToGroupKey(contentKey, groupInfo.groupPublicKey)
                if (null != key) {
                    GroupInfoDataManager.saveGroupKeyParam(gid, keyContent.version, key.base64Encode().format())

                    val g = GroupInfoDataManager.queryOneGroupInfo(gid)
            ...
            GroupKeyMode.NORMAL_MODE -> {
                val contentKey = keyContent.normalModeKey ?: throw Exception("normal key is null")
    ...
    fun uploadGroupKeys(gid: Long, mid: Long, mode: Int) {
        ...
        GroupManagerCore.prepareUploadGroupKeys(gid, mid, mode)
                .subscribeOn(AmeDispatcher.ioScheduler)
                .observeOn(AmeDispatcher.ioScheduler)
                .flatMap { res ->
                ...
                .observeOn(AmeDispatcher.ioScheduler)
                .flatMap {
                    GroupManagerCore.uploadGroupKeys(gid, mid, it, GroupKeyMode.ofValue(mode))
                            .subscribeOn(AmeDispatcher.ioScheduler)
                }.observeOn(AmeDispatcher.ioScheduler)
                .flatMap {
    ...
    fun checkLastGroupKeyValid(gid: Long) {
        ...
        Observable.create<GroupInfo> {
            ...
            if (groupInfo?.isNewGroup == true) {
                ...
                .observeOn(AmeDispatcher.ioScheduler)
                .flatMap {
                    GroupManagerCore.getGroupLatestKeys(listOf(gid))
                            .subscribeOn(AmeDispatcher.ioScheduler)
                }.observeOn(AmeDispatcher.ioScheduler)
                .subscribe({ lastKey ->
    ...
    fun checkGroupKeyValidState(list: List<Long>) {
        ...
        Observable.just(list)
                ...
                .observeOn(AmeDispatcher.ioScheduler)
                .flatMap {
                    val keyLostList = GroupInfoDataManager.queryGroupInfoList(list)
                            .filter { group ->
                    ...
                    Observable.zip(keyLostList.split(5).map {
                        Observable.just(it)
                                ...
                                .observeOn(AmeDispatcher.ioScheduler)
                                .flatMap { list ->
                                    GroupManagerCore.getGroupLatestKeys(list)
                                            .subscribeOn(AmeDispatcher.ioScheduler)
                                }
                                .observeOn(AmeDispatcher.ioScheduler)
                                .map { lastKey ->
                                    for (i in lastKey.keys ?: listOf()) {
                                        val groupInfo = GroupInfoDataManager.queryOneGroupInfo(i.gid)
                                                ?: continue
                                        try {
                                            saveGroupKey(groupInfo, i)
                                        } catch (e: Exception) {
    ...
    fun uploadEncryptedNameAndNotice(groupId: Long, result: (succeed: Boolean) -> Unit) {
        Observable.create<Pair<Optional<String>, Optional<String>>> {
            val groupInfo = GroupInfoDataManager.queryOneGroupInfo(groupId) ?: throw Exception()
            ...
            Pair(Optional.fromNullable(encryptedName), Optional.fromNullable(encryptedNotice))
                ...
                .flatMap {
                    GroupManagerCore.setEncryptedGroupProfile(groupId, it.first.orNull(), it.second.orNull())
                            .subscribeOn(AmeDispatcher.ioScheduler)
                            .observeOn(AmeDispatcher.ioScheduler)
                            .map { result ->
                                if (result.isSuccess) {
    ...
    fun updateGroupName(gid: Long, name: String, result: (succeed: Boolean, error: String?) -> Unit) {
        Observable.create<String> {
            ...
            it.onComplete()
                ...
                .flatMap {
                    GroupManagerCore.updateGroup(gid, it, null)
                            .subscribeOn(AmeDispatcher.ioScheduler)
                }.map {
                    ALog.i(TAG, "result:${it.code},  msg:${it.msg}")
                    if (it.isSuccess) {
