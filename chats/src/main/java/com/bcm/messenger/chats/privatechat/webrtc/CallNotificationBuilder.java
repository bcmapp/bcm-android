...
public class CallNotificationBuilder {
    ...
    public static Notification getCallInProgressNotification(Context context, int type, @NonNull Recipient recipient) {
        Intent contentIntent = new Intent(context, WebRtcCallService.class);
        contentIntent.setAction(WebRtcCallService.ACTION_OUTGOING_CALL);
        PendingIntent pendingIntent = PendingIntent.getService(context, REQUEST_SERVICE_TOUCH, contentIntent, PendingIntent.FLAG_UPDATE_CURRENT);
