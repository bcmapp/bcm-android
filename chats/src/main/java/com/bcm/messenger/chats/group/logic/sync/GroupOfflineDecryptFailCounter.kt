...
class GroupOfflineDecryptFailCounter {
    ...
    private fun updateFailMessage(gid:Long, state: FailState?) {
        if (null != state && state.counter.get() > 0) {
            ...
            val message = MessageDataManager.queryOneMessage(gid, state.mid, false)?:return
            ...
            MessageDataManager.insertSendMessage(message)
        }
    }
}